...
- **`meta.scope`**: `nodeTypes`/`edgeTypes` (Pflicht; erfasste Typen), optional `roots` (Teilbaum-Begrenzung), `excluded` (ausgeschlossene Teilbaum-Wurzeln, z. B. gescheiterte Fetches — Ausfälle dürfen nie als Löschungen fehlinterpretiert werden, PRD FR-5.7) und `edgeSources` (besuchte Quellknoten-IDs eines Enrichment-Crawls: Vollstand nur für deren ausgehende Kanten der deklarierten `edgeTypes`, keine Knoten-Closure — PRD E30).
