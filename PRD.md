...
| E55 | **ZIP-Bündel werden als Dry-Run validiert**: chronologische Simulation des Tenant-Stands über alle Bündel-Snapshots (inkl. Endpunkt-, Closure-, Projektions- und Registry-Checks) **vor** dem ersten echten Commit; erst danach die pro Snapshot atomaren Commits (FR-6.9a) | 2026-07 |
...
**FR-6.9a Import-Atomarität (E47, E55).** Die Validierung (FR-6.8) läuft **vollständig vor jeder Mutation**. Bei ZIP-Bündeln geschieht sie als **Dry-Run über einen simulierten Tenant-Stand** in chronologischer Reihenfolge (E55): Jeder Bündel-Snapshot wird gegen den simulierten Zustand **nach** Anwendung der früheren geprüft — inklusive Endpunkt-Gültigkeit, Closure-Wirkungen, implizierte-Kanten-Projektion und Snapshots-Registry —, denn spätere Snapshots dürfen legitim von früheren abhängen. Erst nach fehlerfreiem Dry-Run beginnt der echte Commit-Lauf: Jeder Snapshot wird als **eine atomare All-or-nothing-Transaktion** auf die IndexedDB angewendet (Versionsfortschreibung, Kaskaden-Schliessungen E35, Projektion E33 und Snapshots-Registry-Eintrag gemeinsam). Schlägt zur Laufzeit dennoch ein Schritt fehl (Exception, Quota, Abbruch), bleibt der Store für diesen Snapshot unverändert, der Snapshot gilt **nicht** als importiert, der Lauf stoppt vor dem nächsten Snapshot, bereits committete bleiben gültig (Toast weist den Teilerfolg aus) — ein erneuter Import ist gefahrlos möglich.
