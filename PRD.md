...
**FR-6.9a Import-Atomarität (E47).** Die Validierung (FR-6.8) läuft **vollständig vor jeder Mutation** — auch bei ZIP-Bündeln werden erst alle enthaltenen Dateien validiert. Danach wird jeder Snapshot als **eine atomare All-or-nothing-Transaktion** auf die IndexedDB angewendet: Versionsfortschreibung, Kaskaden-Schliessungen (E35), implizierte-Kanten-Projektion (E33) und der Snapshots-Registry-Eintrag committen gemeinsam oder gar nicht. Schlägt irgendein Schritt fehl (Exception, Quota, Abbruch), bleibt der Tenant-Store unverändert und der Snapshot gilt **nicht** als importiert — ein erneuter Import ist danach gefahrlos möglich. Mehrere Snapshots eines Bündels werden sequenziell chronologisch angewendet; ein Fehler stoppt vor dem nächsten Snapshot, bereits committete bleiben gültig (der Toast weist den Teilerfolg aus).
