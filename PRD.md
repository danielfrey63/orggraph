...
Die ungelöste Frage «Wie signalisieren wir beim erneuten Crawl, dass ein Knoten, eine Kante oder ein Property nicht mehr vorhanden oder geändert ist?» wird über **Scope-deklarierte Vollstands-Semantik** gelöst — ohne Tombstones im Datenformat:
...
**FR-6.1 Zweiphasig mit Gate.** **Phase A (Analyse):** read-only Inventur aller vorkommenden Knoten- und Beziehungs-Arten über alle Folgeseiten (Überschriften, Tabellenspalten, Attribut-Kategorien, Linkarten), Abgleich gegen die Registry, Report mit `known` (gemappt) und `untyped` (Kandidaten mit Beispielen und Häufigkeit) — dann **Stopp**. **Gate:** pro `untyped`-Eintrag entscheidet Mensch/LLM: bekanntem Typ zuordnen / neuen Typ in die Registry aufnehmen / als Property führen / ignorieren. **Phase B (Voll-Crawl):** vollständige Extraktion ausschliesslich über bekannte Typen. Nichts bleibt still ungetypt; Unklares wird als `props.unclassified` markiert.
