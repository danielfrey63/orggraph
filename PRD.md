...
19. **Quellen-Partition (E37):** Ein Voll-Snapshot der Quelle B schliesst keine Identität, deren offene Version nur von Quelle A geliefert/bestätigt wurde; mit `scope.authoritativeForSources: ["*"]` schliesst er sie.
...
22. **Multi-Source-Merge (E40/E44):** (a) Ein Snapshot der Quelle B ohne die von Quelle A gelieferte Property entfernt diese nicht; (b) liefert B einen abweichenden Wert, entsteht ein ausgewiesener Konflikt, aufgelöst gemäss `SOURCE_PRECEDENCE` bzw. nach Snapshot-Instant; (c) ein **älterer** B-Snapshot, der nach einem jüngeren A-Snapshot importiert wird, überschreibt dessen Property-Wert nicht; (d) dieselben Regeln gelten für ein abweichendes `label`.
