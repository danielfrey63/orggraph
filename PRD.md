...
| E36 | **Snapshot-Identität ≠ Stempel**: Import-Identität = (`source`, `snapshot`, Scope-Fingerprint), Idempotenz über den Inhalts-Hash; verschiedene Quellen oder Scopes kollidieren nie, und die Chronologie-Regel gilt **pro (Quelle, Scope)** (FR-6.9) | 2026-07 |
...
**FR-6.8 Validierung.** Vor dem Diff: `meta.snapshot` entspricht der UTC-Minute von `meta.crawledAt` (E50); Snapshots mit `scope.roots` deklarieren zwingend `scope.edgeSources` (E59), und **jede `edgeSources`-ID muss als Knoten-Record im Snapshot geliefert sein** (Besucht-Beweis — ein Minimal-Record genügt, E61; IDs ohne geliefertes Record werden abgewiesen, damit eine Datei nie gezielt fremde Kanten über erfundene `edgeSources` schliessen kann); Schema ist Teilmenge der im Tenant bekannten Registry; Kanten-Endpunkte existieren (im Snapshot oder im Bestand), respektieren `from`/`to` und sind **zum Snapshot-Instant gültig** (E35): entweder als Record im Snapshot enthalten oder mit offener Version im Bestand. **Reaktivierung ist scope-gebunden:** Eine geschlossene Identität wird nur wieder geöffnet, wenn sie im **Node-Scope** des Snapshots liegt (Vollstands-Anspruch); ein Out-of-Scope-Stub (E61) reaktiviert nie — eine Kante oder Referenz-Property auf eine geschlossene, nur als Stub gelieferte Identität wird mit Details abgewiesen. Knoten-Referenz-Properties (FR-4.7) zeigen auf existierende Knoten des deklarierten Typs; fehlt auf einer Kante der Wert einer deklarierten `identityProp`, ist das ein **Importfehler** (E15); `labelProp`-Auflösungsfehler werden als Warnung gemeldet (FR-4.2b); implizierte Kanten werden materialisiert und die Konsistenz-Invariante geprüft (FR-4.8); Knoten-IDs **und Kanten-Identitäten** (`type`+`source`+`target`+`identityProps`) sind pro Snapshot eindeutig — identische Duplikate werden dedupliziert, widersprüchliche Duplikate (gleiche Identität, abweichende props) sind ein Importfehler mit Report beider Records; Zyklen-Warnung pro definierter View: Bilden die transitiven Selbst-Hops ihres Pfads im neuen Bestand Zyklen, wird mit Details gewarnt (kein stiller Drop; die BFS bleibt robust, FR-7.2).

**FR-6.9 Fortschreibung.** Diff nach FR-5.6 gegen den Tenant-Store, Versionsfortschreibung, Plausibilitäts-Gate (FR-5.7), Toast-Zusammenfassung (FR-5.8). **Snapshot-Identität (E36):** Die Snapshots-Registry im Tenant-Store verzeichnet pro Import `source`, Stempel, Scope-Fingerprint (Hash der `scope`-Deklaration), `registryVersion` und einen **Inhalts-Hash**. Die Import-Identität eines Snapshots ist das Tripel **(`source`, `snapshot`, Scope-Fingerprint)** — der Fingerprint wird über den **semantisch kanonisierten, deklarativen** Scope gebildet (E49): Defaults angewandt (fehlendes `traversalEdgeTypes` = `edgeTypes`), Mengen-Arrays dedupliziert und sortiert, fehlende optionale Felder einheitlich normalisiert — und die lauf-abhängigen Beobachtungsfelder **`edgeSources` und `excluded` bleiben aussen vor** (sie beschreiben, was ein konkreter Lauf besucht bzw. nicht geschafft hat, nicht den Crawl-Job; sonst würde jeder Lauf eine neue Chronologie-Partition eröffnen und ein alter Stand könnte die Abweisung umgehen). So ergeben syntaktisch verschiedene, semantisch gleiche Job-Deklarationen denselben Fingerprint: Ein Snapshot ist nur dann «bereits importiert» (No-op, **Idempotenz**), wenn zum selben Tripel derselbe Inhalts-Hash vorliegt; dasselbe Tripel mit **abweichendem** Inhalt wird als Konflikt mit Details abgewiesen (nie stiller No-op). Snapshots mit verschiedener Quelle **oder** verschiedenem Scope-Fingerprint kollidieren nie — zwei unterschiedlich gescope-te Crawls derselben Quelle in derselben Minute sind legitim; der Stempel allein dient nur der zeitlichen Ordnung. **Chronologie pro Quelle und Scope:** Snapshots desselben (`source`, Scope-Fingerprint)-Paars müssen chronologisch importiert werden; ein älterer Stempel als der jüngste importierte desselben Paars wird mit Hinweis abgewiesen. Quellen und Scopes untereinander sind zeitlich unabhängig. **Bewusster Entscheid:** Historische Backfills desselben (Quelle, Scope)-Paars sind damit unzulässig — wer ein Archiv nachträglich einspielen will, importiert chronologisch von vorn (frischer Tenant); die historischen Einfüge-Regeln (FR-5.2b, E54) gelten ausschliesslich **quellen- oder scope-übergreifend**, nie als Umgehung dieser Abweisung.
...
18. **Snapshot-Identität (E36):** (a) Zwei Snapshots verschiedener Quellen mit identischem Stempel werden beide importiert; (b) der Re-Import eines inhaltsgleichen Snapshots ist ein No-op; (c) derselbe Stempel derselben Quelle mit **gleichem Scope-Fingerprint** und abweichendem Inhalt wird als Konflikt abgewiesen; (d) derselbe Stempel derselben Quelle mit **anderem Scope-Fingerprint** wird importiert; (e) ein älterer Stempel wird nur innerhalb desselben (Quelle, Scope)-Paars abgewiesen; (f) zwei semantisch gleiche, syntaktisch verschiedene `scope`-Deklarationen ergeben denselben Fingerprint (E49).
