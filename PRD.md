...
| E31 | **`harvest-spec.json` ist schema-validiert** (`schema/harvest-spec.schema.json`, AK 11); Phase B verweigert die Ausführung, wenn die referenzierte Spec fehlt, nicht validiert oder die Versionen nicht übereinstimmen (FR-6.1c) | 2026-07 |
...
| E59 | **Bei rooted Snapshots gelten alle gelieferten Knoten implizit als `edgeSources`**: So schliessen auch die Alt-Kanten von Knoten, die neu in den Teilbaum gewandert sind — deren alte Kanten lägen sonst ausserhalb des Pre-Import-Scopes und blieben still stehen (FR-5.5a) | 2026-07 |
...
| NFR-6 | Pseudonymisierung wirkt überall, wo Labels erscheinen (Graph, Legenden, Tooltips, Suche, aufgelöste Referenz-Labels, Export) — **fail-closed** für jeden sichtbaren Typ, auch ohne `pseudonymize`-Capability (E48, FR-8.5). |
...
27. **Fail-closed-Pseudonymisierung (E48):** Bei aktivem Pseudo-Modus zeigt ein sichtbarer Typ ohne `pseudonymize`-Capability in Graph, Suche, Tooltip, Legende, Referenz-Anzeige und Export ausschliesslich den Fallback `<Typname> N`, nie ein echtes Label.
...
36. **Move-in bei rooted Crawls (E59):** Wandert ein Knoten in den Teilbaum eines rooted Snapshots und wird dort geliefert, schliesst der Import seine veralteten Kanten der deklarierten `edgeTypes` (z. B. zur alten OE), obwohl der Knoten vor dem Import nicht im Root-Scope lag.
