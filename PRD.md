...
| E43 | **`excluded` formal definiert** für rooted UND root-freie Snapshots: Ausschluss-Traversal von jeder excluded-Wurzel abwärts über `traversalEdgeTypes` (Default `edgeTypes`), gegen die Kantenrichtung; die Wurzeln selbst und alles nur über sie Erreichbare fällt aus dem Node-Scope, Kanten mit `source` im Ausschluss-Bereich aus dem Kanten-Scope (FR-5.5a) | 2026-07 |
...
| E50 | **Snapshot-Instants sind UTC**: `meta.snapshot` ist die UTC-normalisierte Minute von `meta.crawledAt`; der Import validiert die Konsistenz — zeitzonen-/DST-sichere Ordnung und Konfliktauflösung (FR-5.2a, FR-6.8) | 2026-07 |
...
23. **Excluded-Schutz im root-freien Vollstand (E43):** Ein root-freier Voll-Snapshot mit einem gescheiterten Teilbaum in `scope.excluded` schliesst keinen Knoten und keine Kante innerhalb dieses Teilbaums, obwohl sie im Snapshot fehlen.
