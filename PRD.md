...
| E36 | **Snapshot-Identität ≠ Stempel**: Dedup-Schlüssel der Import-Idempotenz ist der Inhalts-Hash plus (`source`, `snapshot`, Scope-Fingerprint, `registryVersion`); der Stempel allein ordnet nur zeitlich, und die Chronologie-Regel gilt **pro Quelle** (FR-6.9) | 2026-07 |
...
**FR-6.9 Fortschreibung.** Diff nach FR-5.6 gegen den Tenant-Store, Versionsfortschreibung, Plausibilitäts-Gate (FR-5.7), Toast-Zusammenfassung (FR-5.8). **Snapshot-Identität (E36):** Die Snapshots-Registry im Tenant-Store verzeichnet pro Import `source`, Stempel, Scope-Fingerprint (Hash der `scope`-Deklaration), `registryVersion` und einen **Inhalts-Hash**. Ein Snapshot ist nur dann «bereits importiert» (No-op, **Idempotenz**), wenn der Inhalts-Hash übereinstimmt; derselbe Stempel derselben Quelle mit **abweichendem** Inhalt wird als Konflikt mit Details abgewiesen (nie stiller No-op), und Snapshots verschiedener Quellen oder Scopes kollidieren nicht — der Stempel allein dient nur der zeitlichen Ordnung. **Chronologie pro Quelle:** Snapshots derselben `source` müssen chronologisch importiert werden; ein älterer Stempel als der jüngste importierte derselben Quelle wird mit Hinweis abgewiesen. Quellen untereinander sind zeitlich unabhängig.
...
18. **Snapshot-Identität (E36):** (a) Zwei Snapshots verschiedener Quellen mit identischem Stempel werden beide importiert; (b) der Re-Import eines inhaltsgleichen Snapshots ist ein No-op; (c) derselbe Stempel derselben Quelle mit abweichendem Inhalt wird als Konflikt abgewiesen; (d) ein älterer Stempel wird nur innerhalb derselben Quelle abgewiesen.
