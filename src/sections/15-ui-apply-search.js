...
export async function loadAttributesFromFile(file) {
  try {
    ...
    // Prefix attribute names with the category (filename without extension)
    // so the legend groups them hierarchically, same as loadAttributesFromUrl
    const category = file.name.replace(/\.[^/.]+$/, '');
    const compositeAttributes = new Map();
    for (const [identifier, attrs] of attributes.entries()) {
      const m = new Map();
      for (const [name, value] of attrs.entries()) m.set(`${category}::${name}`, value);
      compositeAttributes.set(identifier, m);
    }
    ...
    for (const [identifier, attrs] of compositeAttributes.entries()) {
    ...
    // Register composite types with category-consistent colors (same as loadAttributesFromUrl)
    ...
    for (const type of types) {
      ...
      if (!attributeTypes.has(composite)) {
        const color = colorForCategoryAttribute(category, type, existingInCategory + typeOrdinal);
        attributeTypes.set(composite, color);
        activeAttributes.add(composite); // Neue Attribute standardmäßig aktivieren
      }
      typeOrdinal++;
    }
