...
- `.legend-row` hat einen unscoped Basis-Block in styles.css; `.legend-list` und `#viewsLegend` steuern nur Deltas bei — keine zweite Basis-Definition anlegen
...
19. Hardcodierte Werte im Export-Stylesheet (03), die in styles.css als Token existieren: jeder Literal-Wert in `buildExportStylesheet`, der nicht durch `cssVar(…)` läuft (Verhaltensregeln wie `display: none` ausgenommen).

## Known legitimate exceptions (zuletzt hinterfragt 2026-08-30, Runde 4)
